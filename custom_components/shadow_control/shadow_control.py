...
from homeassistant.const import CONF_NAME, STATE_CLOSED, STATE_OPEN
...
class ShadowControl(CoverEntity):
    ...
    STATE_SHADOW_HORIZONTAL_NEUTRAL_TIMER_RUNNING = "SHADOW_HORIZONTAL_NEUTRAL_TIMER_RUNNING"
    ...
    STATE_DAWN_HORIZONTAL_NEUTRAL_TIMER_RUNNING = "DAWN_HORIZONTAL_NEUTRAL_TIMER_RUNNING"
    ...
    async def async_update(self) -> None:
        ...
        shadow_handling_active = self.hass.states.get(self._shadow_handling_activation_entity_id)
        dawn_handling_active = self.hass.states.get(self._dawn_handling_activation_entity_id)
        ...
        try:
            ...
            self._current_brightness_dawn = float(brightness_dawn.state) if brightness_dawn else None
            self._is_locked = (lock_state.state.lower() == "locked") if lock_state else False
    ...
    async def _perform_state_handling(self):
        ...
        if new_state != self._current_shutter_state:
            self._debug(True, f"State changed from {self._current_shutter_state} to {new_state}")
            ...
            if hasattr(self, method_name):
                ...
                if next_state and next_state != self._current_shutter_state:
                    self._debug(True, f"State handling requested transition to {next_state}")
    ...
    async def _calculate_state(self) -> str:
        ...
        if (await self._is_shadow_handling_activated() and not await self._is_in_sun()) or \
        ...
        elif await self._is_dawn_handling_activated() and not await self._is_dawn_active():
            new_state = await self._get_appropriate_dawn_open_state(current_state)
        else:
            new_state = self.STATE_NEUTRAL

        self._debug(True, f"=== Calculated new state: {new_state} (was: {current_state}) ===")
        return new_state
    ...
    async def _get_input_value(self, config_key: str) -> any:
        ...
        if config_key.endswith("_entity"):
            ...
            return getattr(self, attribute_key) if hasattr(self, attribute_key) else None
    ...
    async def _start_timer(self, delay: float) -> None:
        ...
        self._debug(True, f"Timer started for {delay} seconds. Finish time: {self._timer_finish_time}")
    ...
    async def _calculate_shutter_height(self) -> float | None:
        ...
        if elevation is None or elevation_min is None or elevation_max is None or shadow_max_height is None:
            return None

        # Placeholder for actual calculation logic
        effective_elevation = elevation # For now, use raw elevation
        height_percent = 100 - ((effective_elevation - elevation_min) / (elevation_max - elevation_min)) * shadow_max_height
    ...
    async def _calculate_shutter_angle(self) -> float | None:
        ...
        if azimut is None or facade_angle is None or facade_offset_start is None or facade_offset_end is None or angle_offset is None or min_shutter_angle is None or shadow_max_angle is None:
            return None

        relative_azimut = (azimut - facade_angle + 360) % 360
        if facade_offset_start <= relative_azimut <= (360 + facade_offset_end) % 360:
            angle_percent = min_shutter_angle + ((relative_azimut - facade_offset_start) / (facade_offset_end - facade_offset_start + 360 if facade_offset_end < facade_offset_start else facade_offset_end - facade_offset_start)) * (shadow_max_angle - min_shutter_angle)
    ...
    async def _position_shutter(
        self, height_percent: float | None, angle_percent: float | None, direction: int, force: bool, stop_timer: bool = False
    ) -> None:
        ...
        if self._initial_lbs_run_finished and not await self._is_lbs_locked_in_either_way():
            ...
            if height_percent is not None:
                self._target_position = int(round(height_percent / self._shutter_height_stepping) * self._shutter_height_stepping) if self._shutter_height_stepping > 0 else int(round(height_percent))
                await self._send_by_change("position", self._target_position)
            if angle_percent is not None:
                self._target_tilt = int(round(angle_percent / self._shutter_angle_stepping) * self._shutter_angle_stepping) if self._shutter_angle_stepping > 0 else int(round(angle_percent))
    ...
    async def _send_by_change(self, attribute: str, value: any) -> None:
        ...
        if current_value != value:
            self._debug(True, f"Attribute '{attribute}' changed from '{current_value}' to '{value}', updating...")
    ...
    async def _is_in_sun(self) -> bool:
        ...
        self._debug(True, f"Prüfe Sonneneinfall: Azimut im Bereich: {self._sun_illuminates_facade}, Elevation im Bereich: {is_elevation_in_range}")
    ...
    async def _check_if_facade_is_in_sun(self) -> None:
        ...
        if azimut is None or facade_angle is None or facade_offset_start is None or facade_offset_end is None or min_elevation is None or max_elevation is None or current_elevation is None:
            self._debug(False, "Nicht alle erforderlichen Sonnen- oder Fassadendaten verfügbar für die Prüfung des Sonneneinfalls.")
            return
        ...
        if is_azimut_in_range:
            ...
            effective_elevation = await self._calculate_effective_elevation(current_elevation, azimut)
        else:
        ...
        if isinstance(effective_elevation, (int, float)) and await self._get_input_value("elevation_min") <= effective_elevation <= await self._get_input_value("elevation_max"):
    ...
    async def _is_dawn_active(self) -> bool:
        ...
        return brightness_dawn is not None and dawn_threshold_close is not None and brightness_dawn < dawn_threshold_close
    ...
    async def _handle_state_neutral(self) -> str:
        ...
        if await self._is_lbs_locked_in_either_way():
            self._debug(True, f"Zustand {self.STATE_NEUTRAL}: LBS ist gesperrt, keine Aktion.")
            return self.STATE_NEUTRAL
        ...
        if await self._is_in_sun() and await self._is_shadow_handling_activated():
            ...
            if current_brightness is not None and shadow_threshold_close is not None and current_brightness > shadow_threshold_close and shadow_close_delay is not None:
                self._debug(True, f"Zustand {self.STATE_NEUTRAL}: Helligkeit ({current_brightness}) über Schatten-Schwellwert ({shadow_threshold_close}), starte Timer für {self.STATE_SHADOW_FULL_CLOSE_TIMER_RUNNING} ({shadow_close_delay}s)")
        ...
        if await self._is_dawn_handling_activated():
            ...
            if dawn_brightness is not None and dawn_threshold_close is not None and dawn_brightness < dawn_threshold_close and dawn_close_delay is not None:
                self._debug(True, f"Zustand {self.STATE_NEUTRAL}: Dämmerungsbehandlung aktiv und Helligkeit ({dawn_brightness}) unter Dämmerungs-Schwellwert ({dawn_threshold_close}), starte Timer für {self.STATE_DAWN_FULL_CLOSE_TIMER_RUNNING} ({dawn_close_delay}s)")
        ...
        if neutral_height is not None and neutral_angle is not None:
            ...
            self._debug(True, f"Zustand {self.STATE_NEUTRAL}: Bewege Behang in Neutralposition ({neutral_height}%, {neutral_angle}%).")
        return self.STATE_NEUTRAL
    ...
    async def _handle_state_shadow_full_close_timer_running(self) -> str:
        ...
        if await self._is_lbs_locked_in_either_way():
            self._debug(True, f"Zustand {self.STATE_SHADOW_FULL_CLOSE_TIMER_RUNNING}: LBS ist gesperrt, keine Aktion.")
            return self.STATE_SHADOW_FULL_CLOSE_TIMER_RUNNING
        ...
        if await self._is_in_sun() and await self._is_shadow_handling_activated():
            ...
            if current_brightness is not None and shadow_threshold_close is not None and current_brightness > shadow_threshold_close:
                if await self._is_timer_finished():
                    ...
                    if target_height is not None and target_angle is not None:
                        ...
                        self._debug(True, f"Zustand {self.STATE_SHADOW_FULL_CLOSE_TIMER_RUNNING}: Timer abgelaufen, Helligkeit hoch genug, fahre in Vollschatten ({target_height}%, {target_angle}%) und gehe zu {self.STATE_SHADOW_FULL_CLOSED}")
                        return self.STATE_SHADOW_FULL_CLOSED
                    else:
                        self._debug(False, f"Zustand {self.STATE_SHADOW_FULL_CLOSE_TIMER_RUNNING}: Fehler beim Berechnen der Schattenhöhe oder des Winkels, bleibe in {self.STATE_SHADOW_FULL_CLOSE_TIMER_RUNNING}")
                        return self.STATE_SHADOW_FULL_CLOSE_TIMER_RUNNING
                else:
                    self._debug(True, f"Zustand {self.STATE_SHADOW_FULL_CLOSE_TIMER_RUNNING}: Warte auf Timer (Helligkeit hoch genug)...")
                    return self.STATE_SHADOW_FULL_CLOSE_TIMER_RUNNING
            else:
                self._debug(True, f"Zustand {self.STATE_SHADOW_FULL_CLOSE_TIMER_RUNNING}: Helligkeit ({current_brightness}) nicht höher als Schwellwert ({shadow_threshold_close}), gehe zu {self.STATE_SHADOW_NEUTRAL}")
                await self._stop_timer()
            ...
            if neutral_height is not None and neutral_angle is not None:
                ...
                self._debug(True, f"Zustand {self.STATE_SHADOW_FULL_CLOSE_TIMER_RUNNING}: Nicht in Sonne oder Schattenmodus deaktiviert, fahre in Neutralposition ({neutral_height}%, {neutral_angle}%) und gehe zu {self.STATE_NEUTRAL}")
                return self.STATE_NEUTRAL
            else:
                self._debug(False, f"Zustand {self.STATE_SHADOW_FULL_CLOSE_TIMER_RUNNING}: Neutrale Höhe oder Winkel nicht konfiguriert, gehe zu {self.STATE_NEUTRAL}")
                return self.STATE_NEUTRAL

        # Entsprechung zu LB_LBSID_positionShutterWithPreviousValues
        self._debug(True, f"Zustand {self.STATE_SHADOW_FULL_CLOSE_TIMER_RUNNING}: Beibehalte vorherige Position.")
        return self.STATE_SHADOW_FULL_CLOSE_TIMER_RUNNING

    async def _handle_state_shadow_full_closed(self) -> str:
        """Implementierung der Logik für den Zustand SHADOW_FULL_CLOSED."""
        if await self._is_lbs_locked_in_either_way():
            self._debug(True, f"Zustand {self.STATE_SHADOW_FULL_CLOSED}: LBS ist gesperrt, keine Aktion.")
            return self.STATE_SHADOW_FULL_CLOSED

        if await self._is_in_sun() and await self._is_shadow_handling_activated():
            current_brightness = await self._get_input_value("brightness")
            shadow_threshold_close = await self._get_input_value("shadow_threshold_close")
            shadow_open_slat_delay = await self._get_input_value("shadow_open_slat_delay")
            if current_brightness is not None and shadow_threshold_close is not None and shadow_open_slat_delay is not None and current_brightness < shadow_threshold_close:
                self._debug(True, f"Zustand {self.STATE_SHADOW_FULL_CLOSED}: Helligkeit ({current_brightness}) unter Schwellwert ({shadow_threshold_close}), starte Timer für {self.STATE_SHADOW_HORIZONTAL_NEUTRAL_TIMER_RUNNING} ({shadow_open_slat_delay}s)")
                await self._start_timer(shadow_open_slat_delay)
                return self.STATE_SHADOW_HORIZONTAL_NEUTRAL_TIMER_RUNNING
            else:
                self._debug(True, f"Zustand {self.STATE_SHADOW_FULL_CLOSED}: Helligkeit nicht unter Schwellwert, Neuberechnung der Schattenposition.")
                target_height = await self._calculate_shutter_height()
                target_angle = await self._calculate_shutter_angle()
                if target_height is not None and target_angle is not None:
                    await self._position_shutter(
            ...
            if neutral_height is not None and neutral_angle is not None:
                ...
                self._debug(True, f"Zustand {self.STATE_SHADOW_FULL_CLOSED}: Nicht in Sonne oder Schattenmodus deaktiviert, fahre in Neutralposition ({neutral_height}%, {neutral_angle}%) und gehe zu {self.STATE_NEUTRAL}")
                return self.STATE_NEUTRAL
            else:
                self._debug(False, f"Zustand {self.STATE_SHADOW_FULL_CLOSED}: Neutrale Höhe oder Winkel nicht konfiguriert, gehe zu {self.STATE_NEUTRAL}")
                return self.STATE_NEUTRAL

        # Entsprechung zu LB_LBSID_positionShutterWithPreviousValues
        self._debug(True, f"Zustand {self.STATE_SHADOW_FULL_CLOSED}: Beibehalte vorherige Position.")
        return self.STATE_SHADOW_FULL_CLOSED
    ...
    async def _handle_state_shadow_open_shutter_timer_running(self) -> str:
        ...
        if await self._is_lbs_locked_in_either_way():
            self._debug(True, f"Zustand {self.STATE_SHADOW_OPEN_SHUTTER_TIMER_RUNNING}: LBS ist gesperrt, keine Aktion.")
        ...
        if await self._is_timer_finished():
            ...
            if shadow_open_height is not None and shadow_open_angle is not None:
                ...
                self._debug(True, f"Zustand {self.STATE_SHADOW_OPEN_SHUTTER_TIMER_RUNNING}: Timer abgelaufen, fahre in Schatten-Offen Position ({shadow_open_height}%, {shadow_open_angle}%) und gehe zu {self.STATE_SHADOW_PARTIALLY_OPEN}")
                return self.STATE_SHADOW_PARTIALLY_OPEN
            else:
                self._debug(False, f"Zustand {self.STATE_SHADOW_OPEN_SHUTTER_TIMER_RUNNING}: Höhe oder Winkel für Schatten-Offen nicht konfiguriert, gehe zu {self.STATE_NEUTRAL}")
                return self.STATE_NEUTRAL
        else:
            self._debug(True, f"Zustand {self.STATE_SHADOW_OPEN_SHUTTER_TIMER_RUNNING}: Warte auf Timer...")
            return self.STATE_SHADOW_OPEN_SHUTTER_TIMER_RUNNING

    async def _handle_state_shadow_partially_open(self) -> str:
        """Implementierung der Logik für den Zustand SHADOW_PARTIALLY_OPEN."""
        if await self._is_lbs_locked_in_either_way():
            self._debug(True, f"Zustand {self.STATE_SHADOW_PARTIALLY_OPEN}: LBS ist gesperrt, keine Aktion.")
            return self.STATE_SHADOW_PARTIALLY_OPEN

        if await self._is_in_sun() and await self._is_shadow_handling_activated():
            shadow_open_slat_delay = await self._get_input_value("shadow_open_slat_delay")
            await self._start_timer(shadow_open_slat_delay)
            self._debug(True, f"Zustand {self.STATE_SHADOW_PARTIALLY_OPEN}: Sonne scheint immer noch, starte Timer ({shadow_open_slat_delay}s) zum Öffnen der Lamellen und gehe zu {self.STATE_SHADOW_OPEN_SLATS_TIMER_RUNNING}")
            return self.STATE_SHADOW_OPEN_SLATS_TIMER_RUNNING
        elif not await self._is_in_sun() or not await self._is_shadow_handling_activated():
            self._debug(True, f"Zustand {self.STATE_SHADOW_PARTIALLY_OPEN}: Sonne weg oder Schattenmodus deaktiviert, gehe zu {self.STATE_NEUTRAL}")
            return self.STATE_NEUTRAL
        else:
            self._debug(True, f"Zustand {self.STATE_SHADOW_PARTIALLY_OPEN}: Warte weiterhin auf Sonneneinstrahlung...")
            return self.STATE_SHADOW_PARTIALLY_OPEN
    ...
    async def _handle_state_shadow_open_slats_timer_running(self) -> str:
        ...
        if await self._is_lbs_locked_in_either_way():
            self._debug(True, f"Zustand {self.STATE_SHADOW_OPEN_SLATS_TIMER_RUNNING}: LBS ist gesperrt, keine Aktion.")
            return self.STATE_SHADOW_OPEN_SLATS_TIMER_RUNNING
        ...
        if await self._is_timer_finished():
            ...
            if neutral_angle is not None:
                ...
                self._debug(True, f"Zustand {self.STATE_SHADOW_OPEN_SLATS_TIMER_RUNNING}: Timer abgelaufen, öffne Lamellen auf Neutralwinkel und gehe zu {self.STATE_SHADOW_PARTIALLY_OPEN}")
                return self.STATE_SHADOW_PARTIALLY_OPEN
            else:
                self._debug(False, f"Zustand {self.STATE_SHADOW_OPEN_SLATS_TIMER_RUNNING}: Neutraler Winkel nicht konfiguriert, gehe zu {self.STATE_NEUTRAL}")
                return self.STATE_NEUTRAL
        else:
            self._debug(True, f"Zustand {self.STATE_SHADOW_OPEN_SLATS_TIMER_RUNNING}: Warte auf Timer...")
            return self.STATE_SHADOW_OPEN_SLATS_TIMER_RUNNING
    ...
    async def _handle_state_dawn_full_close_timer_running(self) -> str:
        ...
        if await self._is_lbs_locked_in_either_way():
            self._debug(True, f"Zustand {self.STATE_DAWN_FULL_CLOSE_TIMER_RUNNING}: LBS ist gesperrt, keine Aktion.")
            return self.STATE_DAWN_FULL_CLOSE_TIMER_RUNNING
        ...
        if await self._is_dawn_handling_activated():
            ...
            if dawn_brightness is not None and dawn_threshold_close is not None and dawn_brightness < dawn_threshold_close:
                if await self._is_timer_finished():
                    if dawn_height is not None and dawn_angle is not None:
                        ...
                        self._debug(True, f"Zustand {self.STATE_DAWN_FULL_CLOSE_TIMER_RUNNING}: Timer abgelaufen, fahre in volle Dämmerungsposition ({dawn_height}%, {dawn_angle}%) und gehe zu {self.STATE_DAWN_FULL_CLOSED}")
                        return self.STATE_DAWN_FULL_CLOSED
                    else:
                        self._debug(False, f"Zustand {self.STATE_DAWN_FULL_CLOSE_TIMER_RUNNING}: Dämmerungshöhe oder -winkel nicht konfiguriert, bleibe in {self.STATE_DAWN_FULL_CLOSE_TIMER_RUNNING}")
                        return self.STATE_DAWN_FULL_CLOSE_TIMER_RUNNING
                else:
                    self._debug(True, f"Zustand {self.STATE_DAWN_FULL_CLOSE_TIMER_RUNNING}: Warte auf Timer (Helligkeit niedrig genug)...")
                    return self.STATE_DAWN_FULL_CLOSE_TIMER_RUNNING
            else:
                self._debug(True, f"Zustand {self.STATE_DAWN_FULL_CLOSE_TIMER_RUNNING}: Helligkeit ({dawn_brightness}) nicht unter Schwellwert ({dawn_threshold_close}), gehe zu {self.STATE_DAWN_NEUTRAL} und stoppe Timer.")
                await self._stop_timer()
            ...
            if neutral_height is not None and neutral_angle is not None:
                ...
                self._debug(True, f"Zustand {self.STATE_DAWN_FULL_CLOSE_TIMER_RUNNING}: Dämmerungsbehandlung deaktiviert, fahre in Neutralposition ({neutral_height}%, {neutral_angle}%) und gehe zu {self.STATE_NEUTRAL}")
                return self.STATE_NEUTRAL
            else:
                self._debug(False, f"Zustand {self.STATE_DAWN_FULL_CLOSE_TIMER_RUNNING}: Neutrale Höhe oder Winkel nicht konfiguriert, gehe zu {self.STATE_NEUTRAL}")
                return self.STATE_NEUTRAL

        # Entsprechung zu LB_LBSID_positionShutterWithPreviousValues
        self._debug(True, f"Zustand {self.STATE_DAWN_FULL_CLOSE_TIMER_RUNNING}: Beibehalte vorherige Position.")
        return self.STATE_DAWN_FULL_CLOSE_TIMER_RUNNING

    async def _handle_state_dawn_full_closed(self) -> str:
        """Implementierung der Logik für den Zustand DAWN_FULL_CLOSED."""
        if await self._is_lbs_locked_in_either_way():
            self._debug(True, f"Zustand {self.STATE_DAWN_FULL_CLOSED}: LBS ist gesperrt, keine Aktion.")
            return self.STATE_DAWN_FULL_CLOSED
        ...
        if await self._is_dawn_handling_activated():
            ...
            if dawn_brightness is not None and dawn_threshold_close is not None and dawn_brightness > dawn_threshold_close and dawn_open_slat_delay is not None:
                self._debug(True, f"Zustand {self.STATE_DAWN_FULL_CLOSED}: Dämmerungshelligkeit ({dawn_brightness}) über Schwellwert ({dawn_threshold_close}), starte Timer für {self.STATE_DAWN_HORIZONTAL_NEUTRAL_TIMER_RUNNING} ({dawn_open_slat_delay}s)")
                ...
                self._debug(True, f"Zustand {self.STATE_DAWN_FULL_CLOSED}: Dämmerungshelligkeit nicht über Schwellwert, fahre in Dämmerungsposition ({dawn_height}%, {dawn_angle}%).")
                return self.STATE_DAWN_FULL_CLOSED
            else:
                self._debug(False, f"Zustand {self.STATE_DAWN_FULL_CLOSED}: Dämmerungshöhe oder -winkel nicht konfiguriert, bleibe in {self.STATE_DAWN_FULL_CLOSED}")
                return self.STATE_DAWN_FULL_CLOSED
        else:
            neutral_height = await self._get_input_value("height_neutral")
            neutral_angle = await self._get_input_value("angle_neutral")
            if neutral_height is not None and neutral_angle is not None:
                ...
                self._debug(True, f"Zustand {self.STATE_DAWN_FULL_CLOSED}: Dämmerungsbehandlung deaktiviert, fahre in Neutralposition ({neutral_height}%, {neutral_angle}%) und gehe zu {self.STATE_NEUTRAL}")
                return self.STATE_NEUTRAL
            else:
                self._debug(False, f"Zustand {self.STATE_DAWN_FULL_CLOSED}: Neutrale Höhe oder Winkel nicht konfiguriert, gehe zu {self.STATE_NEUTRAL}")
                return self.STATE_NEUTRAL

        # Entsprechung zu LB_LBSID_positionShutterWithPreviousValues
        self._debug(True, f"Zustand {self.STATE_DAWN_FULL_CLOSED}: Beibehalte vorherige Position.")
        return self.STATE_DAWN_FULL_CLOSED
    ...
    async def _handle_state_dawn_open_shutter_timer_running(self) -> str:
        ...
        if await self._is_lbs_locked_in_either_way():
            self._debug(True, f"Zustand {self.STATE_DAWN_OPEN_SHUTTER_TIMER_RUNNING}: LBS ist gesperrt, keine Aktion.")
            return self.STATE_DAWN_OPEN_SHUTTER_TIMER_RUNNING
        ...
        if await self._is_timer_finished():
            ...
            if dawn_open_height is not None and dawn_open_angle is not None:
                ...
                self._debug(True, f"Zustand {self.STATE_DAWN_OPEN_SHUTTER_TIMER_RUNNING}: Timer abgelaufen, fahre in Dämmerungs-Offen Position ({dawn_open_height}%, {dawn_open_angle}%) und gehe zu {self.STATE_DAWN_PARTIALLY_OPEN}")
                return self.STATE_DAWN_PARTIALLY_OPEN
            else:
                self._debug(False, f"Zustand {self.STATE_DAWN_OPEN_SHUTTER_TIMER_RUNNING}: Höhe oder Winkel für Dämmerungs-Offen nicht konfiguriert, gehe zu {self.STATE_NEUTRAL}")
                return self.STATE_NEUTRAL
        else:
            self._debug(True, f"Zustand {self.STATE_DAWN_OPEN_SHUTTER_TIMER_RUNNING}: Warte auf Timer...")
            return self.STATE_DAWN_OPEN_SHUTTER_TIMER_RUNNING

    async def _handle_state_dawn_partially_open(self) -> str:
        """Implementierung der Logik für den Zustand DAWN_PARTIALLY_OPEN."""
        if await self._is_lbs_locked_in_either_way():
            self._debug(True, f"Zustand {self.STATE_DAWN_PARTIALLY_OPEN}: LBS ist gesperrt, keine Aktion.")
            return self.STATE_DAWN_PARTIALLY_OPEN

        if await self._is_dawn_handling_activated() and await self._is_dawn_active():
            dawn_open_slat_delay = await self._get_input_value("dawn_open_slat_delay")
            await self._start_timer(dawn_open_slat_delay)
            self._debug(True, f"Zustand {self.STATE_DAWN_PARTIALLY_OPEN}: Dämmerung aktiv, starte Timer ({dawn_open_slat_delay}s) zum Öffnen der Lamellen und gehe zu {self.STATE_DAWN_OPEN_SLATS_TIMER_RUNNING}")
            return self.STATE_DAWN_OPEN_SLATS_TIMER_RUNNING
        elif not await self._is_dawn_handling_activated() or not await self._is_dawn_active():
            self._debug(True, f"Zustand {self.STATE_DAWN_PARTIALLY_OPEN}: Dämmerungsmodus deaktiviert oder Dämmerung vorbei, gehe zu {self.STATE_NEUTRAL}")
            return self.STATE_NEUTRAL
        else:
            self._debug(True, f"Zustand {self.STATE_DAWN_PARTIALLY_OPEN}: Warte weiterhin auf Ende der Dämmerung...")
            return self.STATE_DAWN_PARTIALLY_OPEN
    ...
    async def _handle_state_dawn_open_slats_timer_running(self) -> str:
        ...
        if await self._is_lbs_locked_in_either_way():
            self._debug(True, f"Zustand {self.STATE_DAWN_OPEN_SLATS_TIMER_RUNNING}: LBS ist gesperrt, keine Aktion.")
            return self.STATE_DAWN_OPEN_SLATS_TIMER_RUNNING
        ...
        if await self._is_timer_finished():
            ...
            if neutral_angle is not None:
                ...
                self._debug(True, f"Zustand {self.STATE_DAWN_OPEN_SLATS_TIMER_RUNNING}: Timer abgelaufen, öffne Lamellen auf Neutralwinkel und gehe zu {self.STATE_DAWN_PARTIALLY_OPEN}")
                return self.STATE_DAWN_PARTIALLY_OPEN
            else:
                self._debug(False, f"Zustand {self.STATE_DAWN_OPEN_SLATS_TIMER_RUNNING}: Neutraler Winkel nicht konfiguriert, gehe zu {self.STATE_NEUTRAL}")
                return self.STATE_NEUTRAL
        else:
            self._debug(True, f"Zustand {self.STATE_DAWN_OPEN_SLATS_TIMER_RUNNING}: Warte auf Timer...")
            return self.STATE_DAWN_OPEN_SLATS_TIMER_RUNNING
    ...
    async def _handle_state_shadow_horizontal_neutral_timer_running(self) -> str:
        ...
        if await self._is_lbs_locked_in_either_way():
            self._debug(True, f"Zustand {self.STATE_SHADOW_HORIZONTAL_NEUTRAL_TIMER_RUNNING}: LBS ist gesperrt, keine Aktion.")
            return self.STATE_SHADOW_HORIZONTAL_NEUTRAL_TIMER_RUNNING

        if await self._is_in_sun() and await self._is_shadow_handling_activated():
            current_brightness = await self._get_input_value("brightness")
            shadow_threshold_close = await self._get_input_value("shadow_threshold_close")
            shadow_open_slat_angle = await self._get_input_value("shadow_open_slat_angle")
            if current_brightness is not None and shadow_threshold_close is not None and current_brightness > shadow_threshold_close:
                self._debug(True, f"Zustand {self.STATE_SHADOW_HORIZONTAL_NEUTRAL_TIMER_RUNNING}: Helligkeit ({current_brightness}) wieder über Schwellwert ({shadow_threshold_close}), gehe zu {self.STATE_SHADOW_FULL_CLOSED} und stoppe Timer.")
                await self._stop_timer()
                return self.STATE_SHADOW_FULL_CLOSED
            else:
                if await self._is_timer_finished():
                    ...
                    if target_height is not None and shadow_open_slat_angle is not None:
                        ...
                        self._debug(True, f"Zustand {self.STATE_SHADOW_HORIZONTAL_NEUTRAL_TIMER_RUNNING}: Timer abgelaufen, fahre auf Höhe {target_height}% mit neutralen Lamellen ({shadow_open_slat_angle}°) und gehe zu {self.STATE_SHADOW_HORIZONTAL_NEUTRAL}")
                        return self.STATE_SHADOW_HORIZONTAL_NEUTRAL
                    else:
                        self._debug(False, f"Zustand {self.STATE_SHADOW_HORIZONTAL_NEUTRAL_TIMER_RUNNING}: Fehler beim Berechnen der Höhe oder Winkel für offene Lamellen nicht konfiguriert, bleibe in {self.STATE_SHADOW_HORIZONTAL_NEUTRAL_TIMER_RUNNING}")
                        return self.STATE_SHADOW_HORIZONTAL_NEUTRAL_TIMER_RUNNING
                else:
                    self._debug(True, f"Zustand {self.STATE_SHADOW_HORIZONTAL_NEUTRAL_TIMER_RUNNING}: Warte auf Timer (Helligkeit nicht hoch genug)...")
                    return self.STATE_SHADOW_HORIZONTAL_NEUTRAL_TIMER_RUNNING
        else:
            neutral_height = await self._get_input_value("height_neutral")
            neutral_angle = await self._get_input_value("angle_neutral")
            if neutral_height is not None and neutral_angle is not None:
                ...
                self._debug(True, f"Zustand {self.STATE_SHADOW_HORIZONTAL_NEUTRAL_TIMER_RUNNING}: Nicht in Sonne oder Schattenmodus deaktiviert, fahre in Neutralposition ({neutral_height}%, {neutral_angle}%) und gehe zu {self.STATE_NEUTRAL}")
                return self.STATE_NEUTRAL
            else:
                self._debug(False, f"Zustand {self.STATE_SHADOW_HORIZONTAL_NEUTRAL_TIMER_RUNNING}: Neutrale Höhe oder Winkel nicht konfiguriert, gehe zu {self.STATE_NEUTRAL}")
                return self.STATE_NEUTRAL

        # Entsprechung zu LB_LBSID_positionShutterWithPreviousValues
        self._debug(True, f"Zustand {self.STATE_SHADOW_HORIZONTAL_NEUTRAL_TIMER_RUNNING}: Beibehalte vorherige Position.")
        return self.STATE_SHADOW_HORIZONTAL_NEUTRAL_TIMER_RUNNING
    ...
    async def _handle_state_shadow_horizontal_neutral(self) -> str:
        ...
        if await self._is_lbs_locked_in_either_way():
            self._debug(True, f"Zustand {self.STATE_SHADOW_HORIZONTAL_NEUTRAL}: LBS ist gesperrt, keine Aktion.")
            return self.STATE_SHADOW_HORIZONTAL_NEUTRAL

        if await self._is_in_sun() and await self._is_shadow_handling_activated():
            current_brightness = await self._get_input_value("brightness")
            shadow_threshold_close = await self._get_input_value("shadow_threshold_close")
            shadow_open_shutter_delay = await self._get_input_value("shadow_open_shutter_delay")
            if current_brightness is not None and shadow_threshold_close is not None and current_brightness > shadow_threshold_close:
                target_height = await self._calculate_shutter_height()
                target_angle = await self._calculate_shutter_angle()
                if target_height is not None and target_angle is not None:
                    ...
                    self._debug(True, f"Zustand {self.STATE_SHADOW_HORIZONTAL_NEUTRAL}: Helligkeit ({current_brightness}) über Schwellwert ({shadow_threshold_close}), fahre in Vollschatten ({target_height}%, {target_angle}%) und gehe zu {self.STATE_SHADOW_FULL_CLOSED}")
                    return self.STATE_SHADOW_FULL_CLOSED
                else:
                    self._debug(False, f"Zustand {self.STATE_SHADOW_HORIZONTAL_NEUTRAL}: Fehler beim Berechnen der Schattenhöhe oder des Winkels, bleibe in {self.STATE_SHADOW_HORIZONTAL_NEUTRAL}")
                    return self.STATE_SHADOW_HORIZONTAL_NEUTRAL
            elif shadow_open_shutter_delay is not None:
                self._debug(True, f"Zustand {self.STATE_SHADOW_HORIZONTAL_NEUTRAL}: Helligkeit nicht über Schwellwert, starte Timer für {self.STATE_SHADOW_NEUTRAL_TIMER_RUNNING} ({shadow_open_shutter_delay}s)")
                await self._start_timer(shadow_open_shutter_delay)
                return self.STATE_SHADOW_NEUTRAL_TIMER_RUNNING
            else:
                self._debug(False, f"Zustand {self.STATE_SHADOW_HORIZONTAL_NEUTRAL}: Helligkeit nicht über Schwellwert und 'shadow_open_shutter_delay' nicht konfiguriert, bleibe in {self.STATE_SHADOW_HORIZONTAL_NEUTRAL}")
                return self.STATE_SHADOW_HORIZONTAL_NEUTRAL
        else:
            neutral_height = await self._get_input_value("height_neutral")
            neutral_angle = await self._get_input_value("angle_neutral")
            if neutral_height is not None and neutral_angle is not None:
                await self._position_shutter(
                    ...
                    0,  # Richtung: Neutral
                    True,
                    True # Stop Timer (falls ein Timer aktiv war)
                )
                self._debug(True, f"Zustand {self.STATE_SHADOW_HORIZONTAL_NEUTRAL}: Nicht in Sonne oder Schattenmodus deaktiviert, fahre in Neutralposition ({neutral_height}%, {neutral_angle}%) und gehe zu {self.STATE_NEUTRAL}")
                return self.STATE_NEUTRAL
            else:
                self._debug(False, f"Zustand {self.STATE_SHADOW_HORIZONTAL_NEUTRAL}: Neutrale Höhe oder Winkel nicht konfiguriert, gehe zu {self.STATE_NEUTRAL}")
                return self.STATE_NEUTRAL

        # Entsprechung zu LB_LBSID_positionShutterWithPreviousValues
        self._debug(True, f"Zustand {self.STATE_SHADOW_HORIZONTAL_NEUTRAL}: Beibehalte vorherige Position.")
        return self.STATE_SHADOW_HORIZONTAL_NEUTRAL
    ...
    async def _handle_state_shadow_neutral_timer_running(self) -> str:
        ...
        if await self._is_lbs_locked_in_either_way():
            self._debug(True, f"Zustand {self.STATE_SHADOW_NEUTRAL_TIMER_RUNNING}: LBS ist gesperrt, keine Aktion.")
            return self.STATE_SHADOW_NEUTRAL_TIMER_RUNNING
        ...
        if await self._is_in_sun() and await self._is_shadow_handling_activated():
            ...
            if current_brightness is not None and shadow_threshold_close is not None and current_brightness > shadow_threshold_close:
                self._debug(True, f"Zustand {self.STATE_SHADOW_NEUTRAL_TIMER_RUNNING}: Helligkeit ({current_brightness}) wieder über Schwellwert ({shadow_threshold_close}), gehe zu {self.STATE_SHADOW_FULL_CLOSED} und stoppe Timer.")
                await self._stop_timer()
                return self.STATE_SHADOW_FULL_CLOSED
            else:
                if await self._is_timer_finished():
                    if height_after_shadow is not None and angle_after_shadow is not None:
                        ...
                        self._debug(True, f"Zustand {self.STATE_SHADOW_NEUTRAL_TIMER_RUNNING}: Timer abgelaufen, fahre in Position nach Schatten ({height_after_shadow}%, {angle_after_shadow}°) und gehe zu {self.STATE_SHADOW_NEUTRAL}")
                        return self.STATE_SHADOW_NEUTRAL
                    else:
                        self._debug(False, f"Zustand {self.STATE_SHADOW_NEUTRAL_TIMER_RUNNING}: Höhe oder Winkel nach Schatten nicht konfiguriert, bleibe in {self.STATE_SHADOW_NEUTRAL_TIMER_RUNNING}")
                        return self.STATE_SHADOW_NEUTRAL_TIMER_RUNNING
                else:
                    self._debug(True, f"Zustand {self.STATE_SHADOW_NEUTRAL_TIMER_RUNNING}: Warte auf Timer (Helligkeit nicht hoch genug)...")
                    return self.STATE_SHADOW_NEUTRAL_TIMER_RUNNING
        else:
            neutral_height = await self._get_input_value("height_neutral")
            neutral_angle = await self._get_input_value("angle_neutral")
            if neutral_height is not None and neutral_angle is not None:
                ...
                self._debug(True, f"Zustand {self.STATE_SHADOW_NEUTRAL_TIMER_RUNNING}: Nicht in Sonne oder Schattenmodus deaktiviert, fahre in Neutralposition ({neutral_height}%, {neutral_angle}%) und gehe zu {self.STATE_NEUTRAL}")
                return self.STATE_NEUTRAL
            else:
                self._debug(False, f"Zustand {self.STATE_SHADOW_NEUTRAL_TIMER_RUNNING}: Neutrale Höhe oder Winkel nicht konfiguriert, gehe zu {self.STATE_NEUTRAL}")
                return self.STATE_NEUTRAL

        # Entsprechung zu LB_LBSID_positionShutterWithPreviousValues
        self._debug(True, f"Zustand {self.STATE_SHADOW_NEUTRAL_TIMER_RUNNING}: Beibehalte vorherige Position.")
        return self.STATE_SHADOW_NEUTRAL_TIMER_RUNNING

    async def _handle_state_shadow_neutral(self) -> str:
        """Implementierung der Logik für den Zustand SHADOW_NEUTRAL."""
        if await self._is_lbs_locked_in_either_way():
            self._debug(True, f"Zustand {self.STATE_SHADOW_NEUTRAL}: LBS ist gesperrt, keine Aktion.")
            return self.STATE_SHADOW_NEUTRAL
        ...
        if await self._is_in_sun() and await self._is_shadow_handling_activated():
            ...
            if current_brightness is not None and shadow_threshold_close is not None and current_brightness > shadow_threshold_close and shadow_close_delay is not None:
                self._debug(True, f"Zustand {self.STATE_SHADOW_NEUTRAL}: Helligkeit ({current_brightness}) über Schatten-Schwellwert ({shadow_threshold_close}), starte Timer für {self.STATE_SHADOW_FULL_CLOSE_TIMER_RUNNING} ({shadow_close_delay}s)")
                await self._start_timer(shadow_close_delay)
                return self.STATE_SHADOW_FULL_CLOSE_TIMER_RUNNING
            elif dawn_handling_active and dawn_brightness is not None and dawn_threshold_close is not None and dawn_brightness < dawn_threshold_close and dawn_close_delay is not None:
                self._debug(True, f"Zustand {self.STATE_SHADOW_NEUTRAL}: Dämmerungsbehandlung aktiv und Helligkeit ({dawn_brightness}) unter Dämmerungs-Schwellwert ({dawn_threshold_close}), starte Timer für {self.STATE_DAWN_FULL_CLOSE_TIMER_RUNNING} ({dawn_close_delay}s)")
                await self._start_timer(dawn_close_delay)
                return self.STATE_DAWN_FULL_CLOSE_TIMER_RUNNING
                ...
                self._debug(True, f"Zustand {self.STATE_SHADOW_NEUTRAL}: Bewege Behang in Position nach Schatten ({height_after_shadow}%, {angle_after_shadow}%).")
                return self.STATE_SHADOW_NEUTRAL
            else:
                self._debug(False, f"Zustand {self.STATE_SHADOW_NEUTRAL}: Höhe oder Winkel nach Schatten nicht konfiguriert, bleibe in {self.STATE_SHADOW_NEUTRAL}")
                return self.STATE_SHADOW_NEUTRAL
        ...
        if neutral_height is not None and neutral_angle is not None:
            ...
            self._debug(True, f"Zustand {self.STATE_SHADOW_NEUTRAL}: Nicht in Sonne oder Schattenmodus deaktiviert und Dämmerung nicht aktiv, fahre in Neutralposition ({neutral_height}%, {neutral_angle}%) und gehe zu {self.STATE_NEUTRAL}")
            return self.STATE_NEUTRAL
        else:
            self._debug(False, f"Zustand {self.STATE_SHADOW_NEUTRAL}: Neutrale Höhe oder Winkel nicht konfiguriert, gehe zu {self.STATE_NEUTRAL}")
            return self.STATE_NEUTRAL

    async def _handle_state_dawn_neutral(self) -> str:
        """Implementierung der Logik für den Zustand DAWN_NEUTRAL."""
        if await self._is_lbs_locked_in_either_way():
            self._debug(True, f"Zustand {self.STATE_DAWN_NEUTRAL}: LBS ist gesperrt, keine Aktion.")
            return self.STATE_DAWN_NEUTRAL
        ...
        if dawn_handling_active:
            if dawn_brightness is not None and dawn_threshold_close is not None and dawn_brightness < dawn_threshold_close and dawn_close_delay is not None:
                self._debug(True, f"Zustand {self.STATE_DAWN_NEUTRAL}: Dämmerungsbehandlung aktiv und Helligkeit ({dawn_brightness}) unter Dämmerungs-Schwellwert ({dawn_threshold_close}), starte Timer für {self.STATE_DAWN_FULL_CLOSE_TIMER_RUNNING} ({dawn_close_delay}s)")
                await self._start_timer(dawn_close_delay)
                return self.STATE_DAWN_FULL_CLOSE_TIMER_RUNNING
            elif is_in_sun and shadow_handling_active and current_brightness is not None and shadow_threshold_close is not None and current_brightness > shadow_threshold_close and shadow_close_delay is not None:
                self._debug(True, f"Zustand {self.STATE_DAWN_NEUTRAL}: Sonne scheint, Schattenbehandlung aktiv und Helligkeit ({current_brightness}) über Schatten-Schwellwert ({shadow_threshold_close}), starte Timer für {self.STATE_SHADOW_FULL_CLOSE_TIMER_RUNNING} ({shadow_close_delay}s)")
                await self._start_timer(shadow_close_delay)
                return self.STATE_SHADOW_FULL_CLOSE_TIMER_RUNNING
                ...
                self._debug(True, f"Zustand {self.STATE_DAWN_NEUTRAL}: Bewege Behang in Position nach Dämmerung ({height_after_dawn}%, {angle_after_dawn}%).")
                return self.STATE_DAWN_NEUTRAL
            else:
                self._debug(False, f"Zustand {self.STATE_DAWN_NEUTRAL}: Höhe oder Winkel nach Dämmerung nicht konfiguriert, bleibe in {self.STATE_DAWN_NEUTRAL}")
                return self.STATE_DAWN_NEUTRAL

        if is_in_sun and shadow_handling_active and current_brightness is not None and shadow_threshold_close is not None and current_brightness > shadow_threshold_close and shadow_close_delay is not None:
            self._debug(True, f"Zustand {self.STATE_DAWN_NEUTRAL}: Sonne scheint, Schattenbehandlung aktiv und Helligkeit ({current_brightness}) über Schatten-Schwellwert ({shadow_threshold_close}), starte Timer für {self.STATE_SHADOW_FULL_CLOSE_TIMER_RUNNING} ({shadow_close_delay}s)")
            await self._start_timer(shadow_close_delay)
            return self.STATE_SHADOW_FULL_CLOSE_TIMER_RUNNING
        ...
        if neutral_height is not None and neutral_angle is not None:
            ...
            self._debug(True, f"Zustand {self.STATE_DAWN_NEUTRAL}: Dämmerungsbehandlung deaktiviert oder nicht die Bedingungen für Schatten, fahre in Neutralposition ({neutral_height}%, {neutral_angle}%).")
            return self.STATE_NEUTRAL
        else:
            self._debug(False, f"Zustand {self.STATE_DAWN_NEUTRAL}: Neutrale Höhe oder Winkel nicht konfiguriert, gehe zu {self.STATE_NEUTRAL}")
            return self.STATE_NEUTRAL
    ...
    async def _handle_state_dawn_neutral_timer_running(self) -> str:
        ...
        if await self._is_lbs_locked_in_either_way():
            self._debug(True, f"Zustand {self.STATE_DAWN_NEUTRAL_TIMER_RUNNING}: LBS ist gesperrt, keine Aktion.")
        ...
        if await self._is_dawn_handling_activated():
            ...
            if dawn_brightness is not None and dawn_threshold_close is not None and dawn_brightness < dawn_threshold_close:
                self._debug(True, f"Zustand {self.STATE_DAWN_NEUTRAL_TIMER_RUNNING}: Dämmerungshelligkeit ({dawn_brightness}) wieder unter Schwellwert ({dawn_threshold_close}), gehe zu {self.STATE_DAWN_FULL_CLOSED} und stoppe Timer.")
                await self._stop_timer()
                return self.STATE_DAWN_FULL_CLOSED
            else:
                if await self._is_timer_finished():
                    if dawn_height is not None and dawn_open_slat_angle is not None:
                        ...
                        self._debug(True, f"Zustand {self.STATE_DAWN_NEUTRAL_TIMER_RUNNING}: Timer abgelaufen, fahre auf Dämmerungshöhe ({dawn_height}%) mit geöffneten Lamellen ({dawn_open_slat_angle}°) und gehe zu {self.STATE_DAWN_NEUTRAL}")
                        return self.STATE_DAWN_NEUTRAL
                    else:
                        self._debug(False, f"Zustand {self.STATE_DAWN_NEUTRAL_TIMER_RUNNING}: Dämmerungshöhe oder Winkel für offene Lamellen nicht konfiguriert, bleibe in {self.STATE_DAWN_NEUTRAL_TIMER_RUNNING}")
                        return self.STATE_DAWN_NEUTRAL_TIMER_RUNNING
                else:
                    self._debug(True, f"Zustand {self.STATE_DAWN_NEUTRAL_TIMER_RUNNING}: Warte auf Timer (Helligkeit nicht niedrig genug)...")
                    return self.STATE_DAWN_NEUTRAL_TIMER_RUNNING
        else:
            neutral_height = await self._get_input_value("height_neutral")
            neutral_angle = await self._get_input_value("angle_neutral")
            if neutral_height is not None and neutral_angle is not None:
                ...
                self._debug(True, f"Zustand {self.STATE_DAWN_NEUTRAL_TIMER_RUNNING}: Dämmerungsbehandlung deaktiviert, fahre in Neutralposition ({neutral_height}%, {neutral_angle}%) und gehe zu {self.STATE_NEUTRAL}")
                return self.STATE_NEUTRAL
            else:
                self._debug(False, f"Zustand {self.STATE_DAWN_NEUTRAL_TIMER_RUNNING}: Neutrale Höhe oder Winkel nicht konfiguriert, gehe zu {self.STATE_NEUTRAL}")
                return self.STATE_NEUTRAL

        # Entsprechung zu LB_LBSID_positionShutterWithPreviousValues
        self._debug(True, f"Zustand {self.STATE_DAWN_NEUTRAL_TIMER_RUNNING}: Beibehalte vorherige Position.")
        return self.STATE_DAWN_NEUTRAL_TIMER_RUNNING
    ...
    async def _handle_state_dawn_horizontal_neutral(self) -> str:
        ...
        if await self._is_lbs_locked_in_either_way():
            self._debug(True, f"Zustand {self.STATE_DAWN_HORIZONTAL_NEUTRAL}: LBS ist gesperrt, keine Aktion.")
        ...
        if await self._is_dawn_handling_activated():
            ...
            dawn_height = await self._get_input_value("dawn_height")
            dawn_open_slat_angle = await self._get_input_value("dawn_open_slat_angle")
            dawn_open_shutter_delay = await self._get_input_value("dawn_open_shutter_delay")

            if dawn_brightness is not None and dawn_threshold_close is not None and dawn_brightness < dawn_threshold_close and dawn_height is not None and dawn_open_slat_angle is not None:
                await self._position_shutter(
                    float(dawn_height),
                    float(dawn_open_slat_angle),
                    0, # Richtung: Neutral
                ...
                self._debug(True, f"Zustand {self.STATE_DAWN_HORIZONTAL_NEUTRAL}: Dämmerungshelligkeit ({dawn_brightness}) unter Schwellwert ({dawn_threshold_close}), fahre auf Dämmerungshöhe ({dawn_height}%) mit geöffneten Lamellen ({dawn_open_slat_angle}°) und gehe zu {self.STATE_DAWN_FULL_CLOSED}")
                return self.STATE_DAWN_FULL_CLOSED
            elif dawn_open_shutter_delay is not None:
                self._debug(True, f"Zustand {self.STATE_DAWN_HORIZONTAL_NEUTRAL}: Dämmerungshelligkeit nicht unter Schwellwert, starte Timer für {self.STATE_DAWN_NEUTRAL_TIMER_RUNNING} ({dawn_open_shutter_delay}s)")
                await self._start_timer(dawn_open_shutter_delay)
                return self.STATE_DAWN_NEUTRAL_TIMER_RUNNING
            else:
                self._debug(False, f"Zustand {self.STATE_DAWN_HORIZONTAL_NEUTRAL}: Dämmerungshelligkeit nicht unter Schwellwert und 'dawn_open_shutter_delay' nicht konfiguriert, bleibe in {self.STATE_DAWN_HORIZONTAL_NEUTRAL}")
                return self.STATE_DAWN_HORIZONTAL_NEUTRAL
        else:
            neutral_height = await self._get_input_value("height_neutral")
            neutral_angle = await self._get_input_value("angle_neutral")
            if neutral_height is not None and neutral_angle is not None:
                ...
                self._debug(True, f"Zustand {self.STATE_DAWN_HORIZONTAL_NEUTRAL}: Dämmerungsbehandlung deaktiviert, fahre in Neutralposition ({neutral_height}%, {neutral_angle}%) und gehe zu {self.STATE_NEUTRAL}")
                return self.STATE_NEUTRAL
            else:
                self._debug(False, f"Zustand {self.STATE_DAWN_HORIZONTAL_NEUTRAL}: Neutrale Höhe oder Winkel nicht konfiguriert, gehe zu {self.STATE_NEUTRAL}")
                return self.STATE_NEUTRAL

        # Entsprechung zu LB_LBSID_positionShutterWithPreviousValues
        self._debug(True, f"Zustand {self.STATE_DAWN_HORIZONTAL_NEUTRAL}: Beibehalte vorherige Position.")
        return self.STATE_DAWN_HORIZONTAL_NEUTRAL
    ...
    async def _handle_state_dawn_horizontal_neutral_timer_running(self) -> str:
        ...
        if await self._is_lbs_locked_in_either_way():
            self._debug(True, f"Zustand {self.STATE_DAWN_HORIZONTAL_NEUTRAL_TIMER_RUNNING}: LBS ist gesperrt, keine Aktion.")
            return self.STATE_DAWN_HORIZONTAL_NEUTRAL_TIMER_RUNNING
        ...
        if await self._is_dawn_handling_activated():
            ...
            if dawn_brightness is not None and dawn_threshold_close is not None and dawn_brightness < dawn_threshold_close:
                self._debug(True, f"Zustand {self.STATE_DAWN_HORIZONTAL_NEUTRAL_TIMER_RUNNING}: Dämmerungshelligkeit ({dawn_brightness}) wieder unter Schwellwert ({dawn_threshold_close}), gehe zu {self.STATE_DAWN_FULL_CLOSED} und stoppe Timer.")
                await self._stop_timer()
                return self.STATE_DAWN_FULL_CLOSED
            else:
                if await self._is_timer_finished():
                    if dawn_height is not None and dawn_open_slat_angle is not None:
                        ...
                        self._debug(True, f"Zustand {self.STATE_DAWN_HORIZONTAL_NEUTRAL_TIMER_RUNNING}: Timer abgelaufen, fahre auf Dämmerungshöhe ({dawn_height}%) mit geöffneten Lamellen ({dawn_open_slat_angle}°) und gehe zu {self.STATE_DAWN_HORIZONTAL_NEUTRAL}")
                        return self.STATE_DAWN_HORIZONTAL_NEUTRAL
                    else:
                        self._debug(False, f"Zustand {self.STATE_DAWN_HORIZONTAL_NEUTRAL_TIMER_RUNNING}: Dämmerungshöhe oder Winkel für offene Lamellen nicht konfiguriert, bleibe in {self.STATE_DAWN_HORIZONTAL_NEUTRAL_TIMER_RUNNING}")
                        return self.STATE_DAWN_HORIZONTAL_NEUTRAL_TIMER_RUNNING
                else:
                    self._debug(True, f"Zustand {self.STATE_DAWN_HORIZONTAL_NEUTRAL_TIMER_RUNNING}: Warte auf Timer (Helligkeit nicht niedrig genug)...")
                    return self.STATE_DAWN_HORIZONTAL_NEUTRAL_TIMER_RUNNING
        else:
            neutral_height = await self._get_input_value("height_neutral")
            neutral_angle = await self._get_input_value("angle_neutral")
            if neutral_height is not None and neutral_angle is not None:
                ...
                self._debug(True, f"Zustand {self.STATE_DAWN_HORIZONTAL_NEUTRAL_TIMER_RUNNING}: Dämmerungsbehandlung deaktiviert, fahre in Neutralposition ({neutral_height}%, {neutral_angle}%) und gehe zu {self.STATE_NEUTRAL}")
                return self.STATE_NEUTRAL
            else:
                self._debug(False, f"Zustand {self.STATE_DAWN_HORIZONTAL_NEUTRAL_TIMER_RUNNING}: Neutrale Höhe oder Winkel nicht konfiguriert, gehe zu {self.STATE_NEUTRAL}")
                return self.STATE_NEUTRAL

        # Entsprechung zu LB_LBSID_positionShutterWithPreviousValues
        self._debug(True, f"Zustand {self.STATE_DAWN_HORIZONTAL_NEUTRAL_TIMER_RUNNING}: Beibehalte vorherige Position.")
        return self.STATE_DAWN_HORIZONTAL_NEUTRAL_TIMER_RUNNING
    ...
    async def _update_sun_state(self):
        ...
        if azimut is None or facade_angle is None or facade_offset_start is None or facade_offset_end is None or min_elevation is None or max_elevation is None or elevation is None:
            self._debug(False, "Kann Sonnenstatus nicht überprüfen: Nicht alle erforderlichen Eingabewerte sind verfügbar.")
            return
        ...
        if 0 <= azimut_calc <= sun_exit_angle_calc:
            ...
            is_in_sun = True
            await self._set_output_value("sun_at_facade_azimut", 1)
            effective_elevation_result = await self._calculate_effective_elevation() # Implementierung folgt
            ...
            is_in_sun = False
            await self._set_output_value("sun_at_facade_azimut", 0)

        await self._set_output_value("effective_elevation", effective_elevation)

        message += f", effektive Elevation {effective_elevation}° für gegebene Elevation von {elevation}°"
        is_between_min_max_elevation = False
        if effective_elevation != "n/a" and min_elevation <= float(effective_elevation) <= max_elevation:
            message += f"° -> im Min-Max-Bereich ({min_elevation}-{max_elevation})"
            is_between_min_max_elevation = True
            await self._set_output_value("sun_at_facade_elevation", 1)
        else:
            message += f"° -> NICHT im Min-Max-Bereich ({min_elevation}-{max_elevation})"
            is_between_min_max_elevation = False
            await self._set_output_value("sun_at_facade_elevation", 0)
